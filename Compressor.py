...
class compressor:
    def __init__(self,mass,freq):
        self.m=mass #corrected mass flow rate (correlation range from 0.02 to 0.133 kg/s)
        self.freq=freq #rpm #normalized corrected rpm (maximum 14000 rpm, range from 0.42 to 1)
    ...
    def pr(self):
        ...
        ratio = np.where(ratio > 5, np.nan, ratio)
        return ratio
    def efficiency(self):
        ...
        eta = np.where(eta < 0, np.nan, eta)
        eta = np.where(eta > 1, np.nan, eta)
        return eta
